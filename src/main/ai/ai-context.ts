...
function buildSystemPrompt(input: AiChatInput): string {
  return [
    ...
    "用简洁中文回复。当前服务器命令调用 run_shell_command；用户明确提及其他已保存服务器时，必须调用 inspect_saved_server，禁止在当前服务器执行 ssh、scp 或跳板命令。",
    "每次调用工具前必须基于已有结果说明下一步理由；已有结果足够回答时直接总结，不要继续调用工具。",
    ...
    "本地策略概要：拒绝空/NUL/超长或引号、转义不完整的命令；高危、复杂或写入命令必须走审批；遇到重复失败必须改变诊断路径，不得反复执行同一命令。",
    `当前模式：${input.mode}。`,
