...
const emit = defineEmits<{
  ...
  setServerColor: [server: ServerConfig, color?: string];
  moveServerToGroup: [server: ServerConfig, groupId?: string];
  createGroup: [name: string];
  updateGroup: [group: ServerGroup, input: Pick<ServerGroup, "name" | "color">];
...
const colorDialog = reactive<{
  open: boolean;
  target: ServerConfig | ServerGroup | null;
  type: "server" | "group";
}>({ open: false, target: null, type: "server" });
...
const menuItems = computed<ContextMenuItem[]>(() =>
  menuTargetType.value === "group"
    ...
    : [
        {
          key: "pin",
          label: (menuTarget.value as ServerConfig)?.isPinned
            ? "取消置顶"
            : "置顶",
        },
        { key: "edit", label: "编辑连接" },
...
function serversInGroup(groupId: string): ServerConfig[] {
  return props.servers.filter(server => server.groupId === groupId);
}
...
function submitGroup(): void {
  ...
  if (groupDialog.editing)
    emit("updateGroup", groupDialog.editing, {
      name,
      color: groupDialog.editing.color,
    });
  else emit("createGroup", name);
  closeGroupDialog();
}
function openMenu(
  event: MouseEvent,
  target: ServerConfig | ServerGroup,
  type: "server" | "group",
): void {
...
function openColorDialog(
  ...
  colorDialog.type = type;
  colorDialog.open = true;
}
function closeColorDialog(): void {
  colorDialog.open = false;
  colorDialog.target = null;
}
function saveColor(color?: string): void {
  const target = colorDialog.target;
  if (!target) return;
  if (colorDialog.type === "server")
    emit("setServerColor", target as ServerConfig, color);
  else {
    const group = target as ServerGroup;
    emit("updateGroup", group, { name: group.name, color });
  }
}
function selectMenuItem(item: ContextMenuItem): void {
  ...
  closeMenu();
  if (item.key === "color") return openColorDialog(target, type);
  if (type === "group") {
    const group = target as ServerGroup;
    if (item.key === "rename") openGroupDialog(group);
    if (item.key === "delete") {
  ...
  <section :class="['panel', 'server-panel', { collapsed }]">
    ...
    <Transition name="panel-slide"
      ><div v-show="!collapsed" id="server-panel-content" class="server-list">
        ...
        <div v-else-if="!hasServers && !groups.length" class="server-empty">
          暂无服务器，点击右上角新增连接
        </div>
        <section
          ...
          <div class="server-group-header">
            <button
              ...
              ><span>{{ serversInGroup(group.id).length }}</span></button
            ><button
              type="button"
          ...
          <div v-show="!isGroupCollapsed(group.id)" class="server-group-list">
            <p
              v-if="!serversInGroup(group.id).length"
              class="server-group-empty">
              将连接拖到此处
            </p>
            <article
    ...
    <ColorPickerDialog
      ...
      :title="colorDialog.type === 'group' ? '分组颜色' : '连接颜色'"
      :color="colorDialog.target?.color"
      @select="saveColor"
      @clear="
        saveColor(undefined);
        closeColorDialog();
    ...
    <DeleteConfirmDialog
      ...
      title="删除分组"
      :message="`确认删除分组“${groupDeleteDialog.group?.name ?? ''}”？其中的连接会保留在未分组列表中。`"
      confirm-label="删除"
      :danger="true"
...
<style scoped>
/* 通过字号和字重区分面板标题与分组标题，面板间分隔统一交给拖拽条绘制。 */
.panel-header {
  flex-basis: 38px;
  height: 38px;
...
.server-group-header:hover > .server-menu-trigger,
.server-item:hover > .server-menu-trigger,
.server-menu-trigger:focus-visible {
  opacity: 1;
}
