...
test('聊天区模型选择器和复杂历史消息不会撑宽 AI 面板', async () => {
  ...
  assert.match(
    source,
    /\.ai-panel\s*\{[\s\S]*?grid-template-columns: minmax\(0, 1fr\);[\s\S]*?width: 100%;[\s\S]*?min-width: 0;[\s\S]*?max-width: none;/,
  )
  assert.match(
    source,
    /\.ai-panel-header,[\s\S]*?\.ai-message-list,[\s\S]*?\.ai-compose\s*\{[\s\S]*?width: 100%;[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;/,
  )
  assert.match(
    source,
    /\.ai-message-list\s*\{[\s\S]*?contain: inline-size;[\s\S]*?overflow-x: hidden;/,
  )
  assert.match(
    source,
    /\.ai-message\s*\{[\s\S]*?width: 100%;[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;[\s\S]*?overflow: hidden;/,
  )
  assert.match(
    source,
    /\.ai-markdown\s*\{[\s\S]*?width: 100%;[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;[\s\S]*?overflow: hidden;/,
  )
  assert.match(
    source,
    /\.ai-command-output\s*\{[\s\S]*?width: 100%;[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;[\s\S]*?overflow: hidden;/,
  )
  assert.match(
    redesignSource,
    /\.ai-message\.assistant\s*\{[\s\S]*?width: 100%;[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;/,
  )
